...
%% computation of final Mining Cost and Processing Cost

CO2_cost_mining = cost_drill + cost_blast + cost_load + cost_haul_to_dump;
mining_cost = cost_increase_dept + CO2_cost_mining;

CO2_cost_processing = (cost_haul_to_plant - cost_haul_to_dump) + cost_proc;
processing_cost = reference_processing_cost + CO2_cost_processing;

max_mining = max(mining_cost)
min_mining = min(mining_cost)

max_processing = max(processing_cost)
min_processing = min(processing_cost)

T = addvars(T,mining_cost,processing_cost);%add variable created to block model
